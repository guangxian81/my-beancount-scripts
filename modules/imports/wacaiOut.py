...
class WaCaiOut(Base):
    ...
    def __init__(self, filename, byte_content, entries, option_map):
        ...
        if re.match("WaCaiOut.*",filename)==None:
            raise RuntimeError('Not WaCaiOut Trade Record!')
        print('Import WaCai: ')
    ...
    def parse(self):
        ...
        for row in reader:
            if i == 0:
                print('yug'+str(row))
            i=i+1
            print("Importing {} at {}".format(row['支出小类'], row['消费日期']))
            meta = {}
            time = dateparser.parse(row['消费日期'])
            # meta['wechat_trade_no'] = row['交易单号']
            # meta['trade_time'] = row['交易时间']
            # meta['timestamp'] = str(time.timestamp()).replace('.0', '')
            account = get_account_by_guess(row['账户'], row['支出小类'], time)
            flag = "*"
            amount_string = row['消费金额'].replace('¥', '')
