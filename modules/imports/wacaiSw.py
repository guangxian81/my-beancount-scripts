...
class WaCaiSw(Base):
    ...
    def __init__(self, filename, byte_content, entries, option_map):
        ...
        if re.match("WaCaiSw.*",filename)==None:
            raise RuntimeError('Not WaCaiSw Trade Record!')
        print('Import WaCaiSw: ')
    ...
    def parse(self):
        ...
        for row in reader:
            if i == 0:
                print('yug'+str(row))
            i=i+1
            print("Importing {} at {}".format(row['转入账户'], row['转账时间']))
            meta = {}
            time = dateparser.parse(row['转账时间'])
            # meta['wechat_trade_no'] = row['交易单号']
            # meta['trade_time'] = row['交易时间']
            # meta['timestamp'] = str(time.timestamp()).replace('.0', '')
            account = get_account_by_guess(row['转出账户'], row['转入账户'], time)
            flag = "*"
            amount_string = row['转入金额'].replace('¥', '')
            ...
            if status == '人民币' :
                ...
                account = get_account_by_guess(
                    row['转出账户'], row['转入账户'], time)
                # if account == "Unknown":
                #	entry = replace_flag(entry, '!')
                data.create_simple_posting(
                entry, account, amount_string, 'CNY')
                data.create_simple_posting(
                    entry, accounts[row['转出账户']], None, None)
